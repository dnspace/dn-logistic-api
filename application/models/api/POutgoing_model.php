...
class POutgoing_model extends CI_Model
{
    var $tbl_outgoing = 'outgoings';
    var $view_outgoing = 'viewoutgoings';
    var $primKey = 'outgoing_id';
    var $indexKey = 'outgoing_num';
    var $order = array('outgoing_num' => 'desc'); // default order
    ...
    function count_all()
    {
        $this->db->from($this->tbl_outgoing);
        $this->db->where('is_deleted', 0);
        return $this->db->count_all_results();
    }
	
	function count_all_by($arrWhere = array(), $type = "AND")
    {
		$total = 0;
        //Flush Param
        $this->db->flush_cache();
    ...
    function get_key_data($param) {
        $this->db->flush_cache();
        $q = $this->db->query("SELECT MAX(RIGHT(outgoing_num,4)) AS idmax FROM ".$this->tbl_outgoing." WHERE is_deleted = 0");
        $kd = ""; //first code
        if($q->num_rows()>0){
    ...
    function get_key_data_ext($param, $digits) {
        $this->db->flush_cache();
        $q = $this->db->query("SELECT MAX(RIGHT(outgoing_num,".$digits.")) AS idmax FROM ".$this->tbl_outgoing." WHERE is_deleted = 0");
        $kd = ""; //first code
        if($q->num_rows()>0){
            foreach($q->result() as $k){
                $tmp = ((int)$k->idmax); //konversi julah nilai digit yang didapat ke integer
...
//        return $param.$kd;
	...
	*/
    ...
    {
        $this->db->trans_start();
        $this->db->insert($this->tbl_outgoing, $dataInfo);
    ...
    {
        $this->db->where($this->indexKey, $id);
        $this->db->update($this->tbl_outgoing, $dataInfo);
     ...
     */
     ...
     {
         $this->db->where($this->indexKey, $id);
         $this->db->delete($this->tbl_outgoing);
