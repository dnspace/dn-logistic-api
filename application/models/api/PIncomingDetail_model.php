...
class PIncomingDetail_model extends CI_Model
{
    var $tbl_incoming_d = 'incomings_detail';
    var $view_incoming = 'viewdetailincomings';
    var $primKey = 'dt_incoming_id';
    ...
    function count_all()
    {
        $this->db->from($this->tbl_incoming_d);
    ...
    function get_key_data($param) {
        $this->db->flush_cache();
        $q = $this->db->query("SELECT MAX(RIGHT(pticket_number,4)) AS idmax FROM ".$this->tbl_incoming_d."");
        $kd = ""; //first code
        if($q->num_rows()>0){
            foreach($q->result() as $k){
                $tmp = ((int)$k->idmax); //string kode diset ke integer dan ditambahkan 1 dari kode terakhir
                $tmp = $tmp+1;
                $kd = str_pad($tmp,4,'0',STR_PAD_LEFT); //kode ambil 6 karakter terakhir
            }
...
//        return $param.$kd;
    ...
    {
        $this->db->trans_start();
        $this->db->insert($this->tbl_incoming_d, $dataInfo);
    ...
    {
        $this->db->where($this->indexKey, $id);
        $this->db->update($this->tbl_incoming_d, $dataInfo);
     ...
     */
     ...
     {
         $this->db->where($this->indexKey, $id);
         $this->db->delete($this->tbl_incoming_d);
