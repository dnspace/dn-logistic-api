...
class PTicketsDetail_model extends CI_Model
{
    var $tbl_tickets_d = 'parts_ticket_detail';
    var $view_detail = 'ViewTicketsDetail';
    var $primKey = 'pdticket_id';
    var $indexKey = 'pticket_number';
    var $order = array('pticket_number' => 'asc'); // default order
    ...
    function count_all()
    {
        $this->db->from($this->tbl_tickets_d);
    ...
    function get_key_data($param) {
        $this->db->flush_cache();
        $q = $this->db->query("SELECT MAX(RIGHT(pticket_number,2)) AS idmax FROM ".$this->tbl_tickets_d."");
        $kd = ""; //first code
        if($q->num_rows()>0){
...
//        return $param.$kd;
    ...
    {
        $this->db->trans_start();
        $this->db->insert($this->tbl_tickets_d, $dataInfo);
    ...
    {
        $this->db->where($this->indexKey, $id);
        $this->db->update($this->tbl_tickets_d, $dataInfo);
     ...
     */
     ...
     {
         $this->db->where($this->indexKey, $id);
         $this->db->delete($this->tbl_tickets_d);
